...
(define (kt-depth posLi s m) ;position list, size, maximum
   (and (and (> m node) (check (car (reverse posLi)) posLi s m))
...
(define (kt-best-first-h point size maximum)
  (if (kt-best-first-hh (list point) size maximum)
      (display "\n")
      (if (< maximum node) 
          (and (display "Maximum trials reached!\n") (set! node 0))
          (and (display "No knight’s tour exists!\n") (set! node 0)))))
...
(define (kt-depth-first point size maximum)
  (if (kt-depth (list point) size maximum)
      ...
      (if (< maximum node) 
          (and (display "Maximum trials reached!\n") (set! node 0))
          (and (display "No knight’s tour exists!\n") (set! node 0)))))
(display "Test for kt-depth-first when n = 30\n")
(for ([i (in-range 0 2)])
  (kt-depth-first (list (random 29) (random 29)) 30 1000000000000000))
(display "-----------------------------------------------\n")
(display "Test for kt-best-first-hquad when n = 30\n")
(for ([i (in-range 0 2)])
  (kt-best-first-hquad (list (random 29) (random 29)) 30 1000000000000000))
(display "-----------------------------------------------\n")
(display "Test for kt-best-first-h when n = 30\n")
(for ([i (in-range 0 2)])
  (kt-best-first-h (list (random 29) (random 29)) 30 1000000000000000))
